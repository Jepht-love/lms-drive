# Graph Report - lms-drive  (2026-07-09)

## Corpus Check
- 401 files · ~602,694 words
...
3. `formatDate()` - 26 edges
4. `useToast()` - 25 edges
5. `createAdminClient()` - 25 edges
6. `CalendarEvent` - 24 edges
7. `recomputeVehicleStatus()` - 17 edges
8. `CalendarResource` - 17 edges
...
- `ReservationPage()` --calls--> `syncReservationToCalendar()`  [INFERRED]
  app/(dashboard)/reservations/[id]/page.tsx → lib/calendar/syncRental.ts
...
- `InfractionActions()` --calls--> `useToast()`  [EXTRACTED]
  app/(dashboard)/incidents/infractions/[id]/InfractionActions.tsx → components/Toast.tsx
...
Cohesion: 0.07
Nodes (28): NotificationsPage(), GET(), businessNow(), syncAlertsToCalendar(), GET(), ALERT_GROUPS, AlertGroup, DashboardPage() (+20 more)
...
Cohesion: 0.16
Nodes (11): CalendarGridProps, SlotContext, CalendarSidebarProps, RESOURCE_PALETTE, SlotContext, MobileCalendarProps, ResourceListProps, ROLE_LABELS (+3 more)

### Community 13 - "PDF contrat & articles légaux"
Cohesion: 0.13
Nodes (17): getFeesTable(), getLegalArticles(), LegalArticlesParams, ContractPDF(), DamagedZone, fmtDate(), fmtDT(), fmtMoney() (+9 more)
...
Cohesion: 0.05
Nodes (27): deleteContract(), deleteReservation(), resetInspection(), updateDepositDeducted(), updateDepositInfo(), updateDepositStatus(), PAYMENT_LABELS, removeReservationFromCalendar() (+19 more)

### Community 18 - "Incidents / infractions / sinistres"
Cohesion: 0.17
Nodes (7): InfractionDetailPage(), Vehicle, INFRACTION_STATUS, infractionTypeLabel(), SINISTRE_STATUS, formatDate(), formatDateRange()
...
Cohesion: 0.19
Nodes (14): prolongReservation(), updateReservationDates(), ArrivalInspectionPage(), EditDatesPanel(), Props, toInputValue(), ProlongReservation(), Props (+6 more)

### Community 25 - "Sync réservation↔calendrier"
Cohesion: 0.21
Nodes (11): PATCH(), enrichEvents(), generateAlertsForEvent(), ReservationForSync, statusesFor(), syncWashTask(), upsertEvent(), GET() (+3 more)
...
Cohesion: 0.12
Nodes (17): AccountingTransactions(), deleteAgency(), DeleteAgencyButton(), COLORS, ICONS, NotificationsList(), ClientForm(), Toast (+9 more)

### Community 28 - "Analyse financière (agrégats)"
Cohesion: 0.32
Nodes (12): aggregate(), buildAnalysisData(), dateLabel(), pctDelta(), previousRange(), Tx, AnalysisPage(), PERIODS (+4 more)
...
Cohesion: 0.11
Nodes (24): buildDamageFlag(), Candidate, computeVehicleNeeds(), dateCandidate(), fmtKm(), groupNeedsForBadges(), kmCandidate(), LastByType (+16 more)

### Community 32 - "Primitives UI (badge/button/input)"
Cohesion: 0.09
Nodes (21): ReservationStatusButtons(), Badge(), BadgeProps, badgeVariants, Button, ButtonProps, buttonVariants, Card (+13 more)
...
Cohesion: 0.20
Nodes (8): calculateExtraKm(), calculateLateFee(), CLEANLINESS_LEVELS, ComputedFees, Props, Step, compressImageToBase64(), defaultDamagePrice()
...
Cohesion: 0.21
Nodes (9): VIEW_BOXES, DAMAGE_TYPE_PRICES, DAMAGE_TYPES, damageTypeLabel(), GRAVITES, INTERIOR_DAMAGE_ITEMS, VEHICLE_ZONES, VehicleView (+1 more)

### Community 51 - "Graphiques comptables"
Cohesion: 0.14
Nodes (6): getCategoryLabel(), periodRange(), AccountingPage(), PERIODS, Props, M
...
Cohesion: 0.31
Nodes (6): deleteVehicle(), RESA_STATUS_LABEL, VehiclePage(), buildLastByType(), getVehicleStatusColor(), getVehicleStatusLabel()
...
Cohesion: 0.27
Nodes (12): transmitInfractionToClient(), generateInvoiceDraft(), markRestitutionInvoiceSent(), nextInvoiceNumber(), renderContractInvoiceAttachment(), sendInvoice(), SupabaseServer, resendTo() (+4 more)
...
Cohesion: 0.21
Nodes (5): expenseNature(), paymentMethodLabel(), MonthlyClosingPage(), MONTHS, formatPrice()
...
## Knowledge Gaps
- **599 isolated node(s):** `version`, `configurations`, `PreToolUse`, `frontend-design@claude-plugins-official`, `skill-creator@claude-plugins-official` (+594 more)
...
- **Why does `createClient()` connect `Info caution réservation` to `Marketing & Campagnes clients`, `Alertes & Notifications`, `Partenariats inter-agences`, `Actions clients (remises/perks)`, `Community 137`, `Écritures & clôtures comptables`, `Pages dashboard (routes)`, `Community 140`, `Community 141`, `Auth & navigation (Sidebar/rôles)`, `Déplacements internes (trajets)`, `Maintenance & immobilisés`, `Suppressions & caution`, `Community 143`, `Incidents / infractions / sinistres`, `Community 145`, `Documents & expiration`, `Facturation (invoices)`, `Création réservation & blacklist`, `Prolongation & édition dates résa`, `Sync réservation↔calendrier`, `Analyse financière (agrégats)`, `Santé maintenance véhicule`, `Actions incidents (accidents/infractions)`, `Paramètres agence & convention`, `Layout dashboard & transitions`, `Fiche client & photos docs`, `Actions documents & sous-catégories`, `Catégories de dépenses`, `Graphiques comptables`, `Disponibilité employé`, `Signalements véhicule (issues)`, `Fiche véhicule & statut`, `Graphique échéances`, `Génération données contrat PDF`, `PDF sinistre`, `Création tâche & push`, `PDF comptable`, `KPI véhicule`, `Email restitution & pièces`, `Actions véhicules`, `Primitive UI Card`, `Route API manager`, `Profil membre`, `Liste ressources (calendrier)`, `Proxy onglets (middleware)`, `Community 102`?**
