...
import { Plus, ChevronRight, FileWarning } from 'lucide-react'
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
        <Link href={`/suivi?tab=infractions${vehicle ? `&vehicle=${vehicle}` : ''}`} className={pill(!status)}>Tous</Link>
        {Object.entries(INFRACTION_STATUS).map(([s, cfg]) => (
          <Link key={s} href={`/suivi?tab=infractions&status=${s}${vQ}`} className={pill(status === s)}>{cfg.label}</Link>
        ))}
      </div>
