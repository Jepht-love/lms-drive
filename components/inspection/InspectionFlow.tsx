      ...
      const damagedZonesPayload = [
        ...Object.entries(damages)
          ...
          .flatMap(([zoneId, entries]) => entries.map(entry => ({
            ...
            photos: entry.photos,
            // Toute zone signalée comme dommage à l'EDL retour est facturable
            // (montant ajustable, 0 si le gérant ne facture pas). Au départ : pas de prix.
            price: type === 'arrivee' ? priceForZone(zoneId) : 0,
          }))),
      ...
      {step === 'signatures' && (() => {
        ...
        if (type === 'arrivee') {
          ...
          retourRecap = {
            ...
            // Facture de restitution : chaque frais ligne par ligne. Toute zone
            // signalée comme dommage au retour donne une ligne (montant ajustable),
            // qu'elle soit nouvelle ou déjà présente au départ — cohérent avec
