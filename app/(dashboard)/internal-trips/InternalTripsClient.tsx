    ...
    <div className="space-y-6">
            ...
            {activeTrips.map(t => (
              <div key={t.id} className="bg-white rounded-2xl border-2 border-green-200 shadow-sm p-4">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <p className="font-bold text-slate-900">{t.vehicle?.plate}</p>
                    <p className="text-xs text-slate-500">{t.vehicle?.brand} {t.vehicle?.model}</p>
                  </div>
                  <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full font-medium capitalize">
                    {t.purpose}
                  </span>
                </div>
                <p className="text-xs text-slate-400">Depuis {formatDateTime(t.start_datetime)}</p>
                <p className="text-xs text-slate-400">KM départ : {t.km_start.toLocaleString('fr-FR')}</p>
                {(isManager || t.user_id === currentUserId) && (
                  <button
                    onClick={() => setEndingTrip(t)}
                    className="mt-3 w-full py-2 bg-slate-800 text-white rounded-xl text-sm font-medium hover:bg-slate-900 transition-colors"
                  >
      ...
      <div>
        ...
        {completedTrips.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-100 p-8 text-center">
            ...
            <p className="text-slate-500 text-sm">Aucun déplacement terminé</p>
          </div>
        ) : (
                ...
                <div key={t.id} className="flex items-center gap-4 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="font-semibold text-slate-900 text-sm">{t.vehicle?.plate}</p>
                      <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full capitalize">{t.purpose}</span>
                    </div>
        ...
        <form action={handleStart} className="space-y-4">
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-black/20 text-sm bg-white"
            >
          ...
          <div>
            ...
            <select name="purpose" required className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-black/20 text-sm bg-white">
              {PURPOSES.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">
              KM départ * {selectedVehicle && <span className="text-slate-400 font-normal">(actuel: {selectedVehicle.current_km.toLocaleString('fr-FR')})</span>}
            </label>
            <input
              type="number"
              name="km_start"
              ...
              className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-black/20 text-sm"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="text" name="purpose_notes" placeholder="Détails..." className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-black/20 text-sm" />
          </div>
      ...
      <Drawer open={!!endingTrip} onClose={() => setEndingTrip(null)} title={`Terminer — ${endingTrip?.vehicle?.plate ?? ''}`}>
        <form action={handleEnd} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">
              KM retour * <span className="text-slate-400 font-normal">(départ: {endingTrip?.km_start.toLocaleString('fr-FR')})</span>
            </label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Péages (€)</label>
              <input type="number" name="tolls_amount" step="0.01" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-black/20 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Dépenses (€)</label>
              <input type="number" name="expenses_amount" step="0.01" className="w-full px-3 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-black/20 text-sm" />
            </div>
          </div>
          {error && <div className="text-sm text-red-600 bg-red-50 rounded-xl px-3 py-2">{error}</div>}
          <button type="submit" disabled={loading} className="w-full py-3 bg-[#111111] text-white rounded-xl font-semibold disabled:opacity-50 transition-colors active:scale-[.97]">
