    ...
    <>
      <div className="grid grid-cols-2 gap-3">
        {DOCS.map(doc => {
          ...
          if (!url) return (
            <div key={doc.key} className="rounded-xl border-2 border-dashed border-slate-200 aspect-[3/2] flex flex-col items-center justify-center gap-1">
              <FileX className="w-5 h-5 text-slate-300" />
              <span className="text-xs text-slate-300 text-center px-2">{doc.label}</span>
            </div>
          )
          return (
            <button
              ...
              className="group relative rounded-xl overflow-hidden border border-slate-200 aspect-[3/2] bg-slate-50 hover:border-blue-400 transition-colors"
            >
              <Image
                src={url}
                alt={doc.label}
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
              <span className="text-sm font-medium text-slate-700">{lightbox.label}</span>
              <button
                onClick={() => setLightbox(null)}
