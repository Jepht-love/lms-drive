    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <select
          ...
          className="text-sm border border-slate-200 rounded-xl px-3 py-2 text-slate-700 focus:outline-none focus:border-slate-400 bg-white"
        >
        ...
        <input
          ...
          className="flex-1 text-sm border border-slate-200 rounded-xl px-3 py-2 text-slate-700 focus:outline-none focus:border-slate-400"
        />
      </div>

      <p className="text-xs text-slate-400">{filtered.length} action{filtered.length > 1 ? 's' : ''}</p>

      <div className="space-y-1 max-h-[28rem] overflow-y-auto">
        {filtered.length === 0 ? (
          <p className="text-sm text-slate-400 py-4 text-center">Aucune action ne correspond</p>
        ) : (
          filtered.map(log => {
            const tone = auditActionTone(log.action)
            const detail = formatAuditDetail(log)
            return (
              <div key={log.id} className="flex items-start gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-50">
                <span className={`w-2 h-2 rounded-full flex-shrink-0 mt-1.5 ${TONE_DOT[tone]}`} />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-slate-800">{auditActionLabel(log.action)}</p>
                  {detail && <p className="text-xs text-slate-500 truncate">{detail}</p>}
                  <p className="text-[11px] text-slate-400 mt-0.5 flex items-center gap-1">
                    <Shield className="w-3 h-3 text-slate-300" /> {userName(log)} · {formatDateTime(log.created_at)}
                  </p>
                </div>
              </div>
            )
